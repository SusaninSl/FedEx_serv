...
def _create_and_send_shipment(
    ...
    *,
    ...
    etd_detail: schemas.ETDDetail | None = None,
    ...
    try:
        tracking_number, label_path = client.create_shipment(
            ...
            etd_detail=etd_detail.dict(exclude_none=True) if etd_detail else None,
            is_return=is_return,
            return_reference=return_reference,
        )
...
def create_shipment(order: schemas.ShipmentCreate, db: Session = Depends(get_db)):
    if order.freight_items:
        raise HTTPException(
            ...
            detail="Freight items must be submitted via /ltl/orders",
        )
...
TEST_SERVICE_TYPES: list[schemas.AllowedService] = [
    "FIP",
    "IPE",
    "FIE",
    "RE",
