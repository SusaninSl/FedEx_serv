...
- Фрахтові сервіси FedEx International Priority/ Economy/ Regional Economy Freight (IPF/IEF/REF) через LTL endpoint `/ltl/orders`.
...
- **Створити відправлення (звичайний ship)**
  ...
  - `etd_documents` — Electronic Trade Documents (ETD) з попереднім аплоадом документів: сервіс викликає Trade Documents Upload API з `workflowName=ETDPreshipment` через `multipart/form-data` (частина `document` + файл `attachment`), отримує `docId` і передає його в `shipmentSpecialServices.etdDetail.attachedDocuments`.

- **Створити LTL-фрахт відправлення (IPF/IEF/REF)**
  ```bash
  curl -X POST "http://localhost:8000/ltl/orders?token=super-secret-token" \
    -H "Content-Type: application/json" \
    -d '{
...
- ETD (Customer Document Upload): документи з `etd_documents` спочатку завантажуються у FedEx Upload Documents (`/upload-documents/v1/documents`, `workflowName=ETDPreShipment`), після чого їх `docId` додаються у `shipmentSpecialServices.etdDetail.attachedDocuments` під час створення відправлення.
- Freight-сервіси IPF/IEF/REF створюються через окремий LTL-ендпоїнт `/ltl/orders` (FedEx LTL Freight API), а не через `/orders`.
